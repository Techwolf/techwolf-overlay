...
multilib_src_install() {
	...
	# install headers in non-conflicting filename space
	mkdir -p "${D}/usr/include/openssl10"
	cp ${BUILD_DIR}/include/openssl/*.h "${D}/usr/include/openssl10"
	# fix #include within the headers
	for x in ${D}/usr/include/openssl10/*.h ; do
	  sed -i -e 's:include <openssl/:include <openssl10/:' "${x}"
	done;
}
