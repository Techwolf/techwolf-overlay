# Copyright 2009 Techwolf Lupindo
# Distributed under the terms of the GNU General Public License v2
# $Header: $

EAPI="2"

inherit eutils mono games subversion

DESCRIPTION="Lightweight client for connecting to Second Life and OpenSim based virtual worlds."
HOMEPAGE="http://radegastclient.org/"
LICENSE="BSD"
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="debug"
RESTRICT="mirror"

RDEPEND="dev-lang/mono
	dev-dotnet/libgdiplus[cairo]"
DEPEND="${RDEPEND}
	dev-dotnet/nant"

src_unpack() {
	# When using svc, S is the directory the checkout is copied into.
	# Set it so it matches the wiki build instrucktions.
	S="${WORKDIR}/radegast"
	ESVN_REPO_URI="http://radegast.googlecode.com/svn/trunk/"
	ESVN_PROJECT="radegast"
	subversion_src_unpack

	S="${WORKDIR}/libopenmetaverse"
	ESVN_REPO_URI="http://libopenmetaverse.googlecode.com/svn/libopenmetaverse/trunk/"
	ESVN_PROJECT="libopenmetaverse"
	subversion_src_unpack

	S="${WORKDIR}"
}

src_compile() {
	cd radegast || die "Failed to cd into radegast"
	./runprebuild.sh  || die "Prebuild failed"
	use debug && nant Debug build
	use debug || nant Release build
}

src_install() {
	cd "${S}/radegast"
	exeinto "${GAMES_DATADIR}/${PN}"
	doexe bin/Radegast.exe || die
	doexe bin/*.so || die
	insinto "${GAMES_DATADIR}/${PN}"
	doins bin/*.dll || die
	doins bin/*.wav || die
	doins bin/*.config || die
	doins bin/*.dylib || die
	doins bin/*.XML || die
	doins bin/*.xml || die
	use debug && doins bin/*.mdb
	insinto "${GAMES_DATADIR}/${PN}/aiml"
	doins bin/aiml/*.aiml || die
	insinto "${GAMES_DATADIR}/${PN}/aiml_config"
	doins bin/aiml_config/*.xml || die
	insinto "${GAMES_DATADIR}/${PN}/openmetaverse_data"
	doins bin/openmetaverse_data/*.xml || die
	doins bin/openmetaverse_data/*.tga || die

	# gentoo specific stuff
	games_make_wrapper "radegast-svn" "mono ./Radegast.exe" "${GAMES_DATADIR}/${PN}"
	# todo: insert/install an icon here when upstream makes one.
	make_desktop_entry "radegast-svn" "radegast-svn"
	prepgamesdirs
}
