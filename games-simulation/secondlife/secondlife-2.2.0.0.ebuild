...
EHG_REPO_URI="http://hg.secondlife.com/viewer-development"
EHG_REVISION="2.2.0-release"
...
src_prepare() {
	...
	epatch "${FILESDIR}"/cmake_package.patch
	# sed -i -e 's:add_subdirectory(eventhost)::' "${WORKDIR}/linden/indra/viewer_components/CMakeLists.txt"
	epatch "${FILESDIR}"/SNOW-651_SLPlugin_LL_TESTS.patch
	epatch "${FILESDIR}"/SNOW-654_integration_LL_TESTS.patch
	epatch "${FILESDIR}"/llqtwebkit_TARGET_NONE_fix.patch
